...
export default function LoanApplicationSystem() {
  ...
  const handleLogin = async (email: string, password: string) => {
    // Login is handled by auth service through the onAuthStateChange listener
  };

  const handleSignup = (name: string, email: string, password: string) => {
    // Signup is handled by auth service through the onAuthStateChange listener
  };
  ...
  const handleLoanSubmit = async (loanData: LoanApplicationInput) => {
    if (!user || !userProfile) {
    ...
    try {
      ...
      await loanService.submitApplication(user.id, loanData);
